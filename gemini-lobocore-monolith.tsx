...
import { 
  User, Lock, Mail, Phone, FileText, Trash2, Edit, Plus, 
  Menu, X, Home, Users, Package, Settings, LogOut, 
  Moon, Sun, ChevronLeft, ChevronRight, Activity, 
  CheckCircle2, XCircle, AlertTriangle, Search, Filter, ShieldCheck, Sparkles,
  Eye, EyeOff, UserCog, Calendar, ArrowDownAZ, ArrowUpZA, SortDesc, Save, Info,
  ChevronDown
} from 'lucide-react';
...
const initialUsers = [
  { id: 1, name: 'Lobo Master', email: 'super@lobocore.com', role: 'super_admin' },
  { id: 2, name: 'Admin Silva', email: 'admin@lobocore.com', role: 'admin' },
  { id: 3, name: 'Cliente Tech', email: 'client@lobocore.com', role: 'client' },
];
...
let mockAppUsers = [
  ...
  { id: 7, name: 'João Cliente', email: 'joao@cliente.com', role: 'client', active: true, createdAt: '2023-07-01T08:00:00Z' },
...
let mockClients = [
  { id: 1, name: 'Tech Solutions SA', email: 'contato@techsol.com', phone: '(11) 99999-1111', document: '12.345.678/0001-90', active: true, createdAt: '2023-01-20T10:00:00Z' },
  { id: 2, name: 'Global Corp', email: 'hello@globalcorp.net', phone: '(11) 98888-2222', document: '98.765.432/0001-10', active: true, createdAt: '2023-03-15T14:00:00Z' },
  { id: 3, name: 'Inovação LTDA', email: 'sac@inovacao.br', phone: '(21) 97777-3333', document: '11.222.333/0001-44', active: false, createdAt: '2023-06-10T11:30:00Z' },
];

let mockProducts = [
  { id: 1, name: 'LoboCore Server Premium', sku: 'LBC-SRV-001', price: 4999.90, stock: 15, category: 'Hardware', description: 'Servidor de alta performance com arquitetura LoboCore.', active: true, createdAt: '2023-01-05T10:00:00Z' },
  { id: 2, name: 'Licença LoboCore Anual', sku: 'LBC-LIC-012', price: 1200.00, stock: 999, category: 'Software', description: 'Licença anual completa para gestão tecnológica.', active: true, createdAt: '2023-02-12T09:00:00Z' },
  { id: 3, name: 'Módulo de IA Avançada', sku: 'LBC-MOD-005', price: 850.50, stock: 50, category: 'Plugin', description: 'Expansão de inteligência artificial para automação de processos.', active: false, createdAt: '2023-04-20T15:00:00Z' },
  { id: 4, name: 'Suporte Enterprise', sku: 'LBC-SUP-001', price: 3000.00, stock: 999, category: 'Serviço', description: 'Suporte 24/7 direto com engenheiros especializados.', active: true, createdAt: '2023-05-30T10:30:00Z' },
];
...
const api = {
  ...
  users: {
    get: async () => { await delay(600); return [...mockAppUsers]; },
    post: async (data) => { 
      await delay(600); 
      const newUser = { id: Date.now(), ...data, active: data.active ?? true, createdAt: new Date().toISOString() }; 
    ...
    put: async (id, data) => { 
      ...
      mockAppUsers[idx] = { ...mockAppUsers[idx], ...data, active: mockAppUsers[idx].active };
      return mockAppUsers[idx];
    },
    patchStatus: async (id, status) => {
      await delay(400);
      const idx = mockAppUsers.findIndex(u => u.id === id);
      mockAppUsers[idx].active = status;
      return mockAppUsers[idx];
    },
    delete: async (id) => {
      await delay(600);
  ...
  clients: {
    get: async () => { await delay(600); return [...mockClients]; },
    post: async (data) => { await delay(600); const newClient = { id: Date.now(), ...data, active: true, createdAt: new Date().toISOString() }; mockClients.push(newClient); return newClient; },
    put: async (id, data) => { 
      await delay(600); 
      const idx = mockClients.findIndex(c => c.id === id);
      mockClients[idx] = { ...mockClients[idx], ...data, active: mockClients[idx].active };
      return mockClients[idx];
    },
    patchStatus: async (id, status) => {
      await delay(400);
      const idx = mockClients.findIndex(c => c.id === id);
      mockClients[idx].active = status;
      return mockClients[idx];
    },
    delete: async (id) => {
      await delay(600);
  ...
  products: {
    get: async () => { await delay(600); return [...mockProducts]; },
    post: async (data) => { await delay(600); const newProd = { id: Date.now(), ...data, active: true, createdAt: new Date().toISOString() }; mockProducts.push(newProd); return newProd; },
    put: async (id, data) => { 
      await delay(600); 
      const idx = mockProducts.findIndex(p => p.id === id);
      mockProducts[idx] = { ...mockProducts[idx], ...data, active: mockProducts[idx].active };
      return mockProducts[idx];
    },
    patchStatus: async (id, status) => {
      await delay(400);
      const idx = mockProducts.findIndex(p => p.id === id);
      mockProducts[idx].active = status;
      return mockProducts[idx];
    },
    delete: async (id) => {
      await delay(600);
...
const BrandImage = ({ type = 'large', className = '' }) => {
  ...
  let src = '';
  if (type === 'large') src = 'https://i.postimg.cc/4dtFHJ77/lobocore-logo-horizontal-large.png';
  if (type === 'compact') src = 'https://i.postimg.cc/ZR3DvTB9/lobocore-logo-horizontal-compact.png';
  if (type === 'icon') src = 'https://i.postimg.cc/yxTK9hgT/lobocore-icone-transparente.png';
...
const UsersView = () => {
  ...
  const roleNames = { 'super_admin': 'Super Admin', 'admin': 'Admin', 'client': 'Client' };
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(u => u.name.toLowerCase().includes(lowerTerm) || u.email.toLowerCase().includes(lowerTerm));
    }
    if (roleFilter !== 'all') result = result.filter(u => u.role === roleFilter);
    if (statusFilter !== 'all') result = result.filter(u => u.active === (statusFilter === 'active'));
    result.sort((a, b) => {
  ...
  const formatDate = (dateString) => new Intl.DateTimeFormat('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' }).format(new Date(dateString));

  if (user.role === 'client') return <div className="text-center p-12 text-slate-500">Acesso Restrito.</div>;
              ...
              {loading ? (
                ...
                filteredUsers.map((u) => (
                  <tr key={u.id} className="bg-white dark:bg-[#141821] hover:bg-slate-50 dark:hover:bg-[#181C26] transition-colors group">
                    ...
                    <td className="px-6 py-4"><Badge variant={u.role === 'super_admin' ? 'purple' : u.role === 'admin' ? 'info' : 'default'}>{roleNames[u.role]}</Badge></td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-slate-500 dark:text-slate-400 text-xs"><div className="flex items-center gap-1.5"><Calendar size={13} /> {formatDate(u.createdAt)}</div></td>
...
const ClientsView = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingClient) {
        await api.clients.put(editingClient.id, formData);
        showToast('Cliente atualizado com sucesso', 'success');
      } else {
        await api.clients.post(formData);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      await api.clients.patchStatus(id, !currentStatus);
      showToast(`Cliente ${!currentStatus ? 'ativado' : 'inativado'}`, 'success');
      setClients(clients.map(c => c.id === id ? { ...c, active: !currentStatus } : c));
    } catch (e) {
      showToast('Erro ao alterar status', 'error');
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    setSaving(true);
    try {
      await api.clients.delete(itemToDelete.id);
  ...
  const filteredClients = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(c => c.name.toLowerCase().includes(low) || c.document.includes(searchTerm));
    }
    if (statusFilter !== 'all') result = result.filter(c => c.active === (statusFilter === 'active'));
    result.sort((a, b) => {
      if (sortBy === 'nameAsc') return a.name.localeCompare(b.name);
      if (sortBy === 'nameDesc') return b.name.localeCompare(a.name);
      if (sortBy === 'oldest') return new Date(a.createdAt) - new Date(b.createdAt);
...
const ProductsView = () => {
  ...
  const loadProducts = async () => {
    ...
    try {
      ...
      setProducts(isClient ? data.filter(p => p.active) : data);
    } catch (e) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingProduct) {
        await api.products.put(editingProduct.id, payload);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    try {
      await api.products.patchStatus(id, !currentStatus);
  ...
  const handleConfirmDelete = async () => {
    ...
    setSaving(true);
    try {
      await api.products.delete(itemToDelete.id);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (statusFilter !== 'all') result = result.filter(p => p.active === (statusFilter === 'active'));
    result.sort((a, b) => {
...
const ProfileView = () => {
  ...
  const fullUser = useMemo(() => {
    return mockAppUsers.find(u => u.email === user.email) || { ...user, active: true, createdAt: new Date().toISOString() };
  }, [user]);

  const [formData, setFormData] = useState({ name: fullUser.name, email: fullUser.email, password: '' });

  const roleNames = { 'super_admin': 'Super Administrador', 'admin': 'Administrador', 'client': 'Cliente' };
  ...
  const handleSave = async (e) => {
    ...
    try {
      await delay(800); 
      const idx = mockAppUsers.findIndex(u => u.email === user.email);
      if (idx !== -1) {
        mockAppUsers[idx].name = formData.name;
        mockAppUsers[idx].email = formData.email;
      }
            ...
            {isEditing ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-slate-50 dark:bg-[#141821] p-4 rounded-lg border border-slate-200 dark:border-white/5 space-y-5">
                      <div><p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Nível de Acesso (Role)</p><Badge variant={fullUser.role === 'super_admin' ? 'purple' : fullUser.role === 'admin' ? 'info' : 'default'}>{roleNames[fullUser.role]}</Badge></div>
                      ...
                      <div><p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Membro Desde</p><p className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center gap-2"><Calendar size={14} className="text-slate-400" /> {formatDate(fullUser.createdAt)}</p></div>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in">
                <div className="space-y-6">
                  ...
                  <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Membro Desde</p><p className="text-slate-900 dark:text-white flex items-center gap-2"><Calendar size={18} className="text-slate-400"/> {formatDate(fullUser.createdAt)}</p></div>
                </div>
                <div className="space-y-6">
                  <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2">Nível de Acesso (Role)</p><Badge variant={fullUser.role === 'super_admin' ? 'purple' : fullUser.role === 'admin' ? 'info' : 'default'} className="text-sm px-3 py-1">{roleNames[fullUser.role]}</Badge>{fullUser.role === 'super_admin' && <p className="text-xs text-slate-400 mt-2 flex items-center gap-1"><ShieldCheck size={14}/> Acesso irrestrito ao sistema.</p>}</div>
...
const Layout = ({ children, currentView, setCurrentView }) => {
  ...
  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: Home, show: user.role === 'super_admin' },
    { id: 'users', label: 'Usuários', icon: UserCog, show: user.role === 'super_admin' },
    { id: 'clients', label: 'Clientes', icon: Users, show: user.role === 'super_admin' || user.role === 'admin' },
    { id: 'products', label: 'Produtos', icon: Package, show: true },
    { id: 'profile', label: 'Meu Perfil', icon: Settings, show: true },
...
export default function App() {
    ...
    const checkAuth = async () => {
      ...
      if (token) {
        try {
          ...
          setUser(data.user);
          if (data.user.role === 'admin') setCurrentView('clients');
          else if (data.user.role === 'client') setCurrentView('products');
        } catch {
          localStorage.removeItem('lobocore_token');
        }
      }
  ...
  const login = async (email, password) => {
    ...
    setUser(data.user);
    if (data.user.role === 'super_admin') setCurrentView('dashboard');
    else if (data.user.role === 'admin') setCurrentView('clients');
  ...
  const renderView = () => {
    if (currentView === 'dashboard' && user.role !== 'super_admin') return user.role === 'admin' ? <ClientsView /> : <ProductsView />;
    if (currentView === 'users' && user.role !== 'super_admin') return user.role === 'admin' ? <ClientsView /> : <ProductsView />;
    if (currentView === 'clients' && user.role === 'client') return <ProductsView />;
    ...
    switch (currentView) {
      ...
      default: return user.role === 'super_admin' ? <DashboardView /> : (user.role === 'admin' ? <ClientsView /> : <ProductsView />);
    }
  };

  if (loadingAuth) {
    return (
      <div className="min-h-screen bg-[#0E121B] flex items-center justify-center flex-col gap-6">
    ...
    <ThemeContext.Provider value={{ theme, toggleTheme }}>
      <AuthContext.Provider value={{ user, setUser, login, logout }}>
        <ToastContext.Provider value={{ showToast }}>
          ...
          {!user ? <LoginView /> : <Layout currentView={currentView} setCurrentView={setCurrentView}>{renderView()}</Layout>}
